...
object controller {
	...
	def listServers(): String = {
		println("listServers function received no tokens.")
			...
			previousServer match {
				...
				case None => {
					...
					nextServer match {
						...
						case None => println("Determined that there is no next server on the hash ring.") // THIS WILL NEVER HAPPEN, SO I'M NOT SURE WHAT TO DO HERE

						case Some(nextServer) => {
							println("Determined that there is some next server on the hash ring.")
